...
export default {
  data () {
    ...
    groupOptions: [],
      page: 1,
        filter: { },
    query: { page: 1, 'order[created_at]': 'desc' },
...
watch: {
  page(it) {
    this.query.page = it
    this.get{{{ titleUcFirst}}}A({ query: this.query })
  },
  search(val) {
  ...
  created{{{ titleUcFirst}}}G(item) {
    ...
    const p = { query: this.query }
    this.get{{{ titleUcFirst}}}A(p)
...
delete{{{ titleUcFirst }}} (item) {
  ...
  const query = item ? { page: 1, 'filter[group]': item } : { page: 1 }
  this.get{{{ titleUcFirst}}}A({ query })
},
onSelect(val) {
